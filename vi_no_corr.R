...
tau_update_ncr <- function(X_mat, y_vec, w_mat, id, uq_id, mu_nu, Sigma_nu, mu_beta, Sigma_beta, psi_taus,
  ...
  hij <- as.vector(err_sq * psi_omegas)
...
omega_update_ncr <- function(X_mat, y_vec, id, uq_id, mu_beta, Sigma_beta, mu_nu, Sigma_nu, mu_inv_sigma, 
  ...
  var_beta <- rowSums((X_mat %*% Sigma_beta) * X_mat)
  err_sq <- fixed^2 + var_beta + Sigma_nuis
  hij <- as.vector(err_sq * psi_taus)
  
  tol_grad <- -0.5 + 0.5 * psi_omegas * hij - mu_inv_sigma * mu_omegas
  tol_hess <- -0.5 * psi_omegas * hij - mu_inv_sigma
...
elbo_cal_ncr <- function(X_mat, y_vec, u_mat, w_mat, mu_beta, Sigma_beta, mu_nu, Sigma_nu, 
                     ...
                     psi_taus, psi_alphas, psi_omegas, 
  ...
  log_p_y <- -0.5 * sum(w_mat %*% mu_tau + mu_omega) - 0.5 * sum(psi_omegas * hij)
  ...
  elbo <- log_p_y + log_p_beta + log_p_tau + log_p_alpha + log_p_nu + log_p_omega + log_p_sigma + log_p_a + 
    log_q_beta + log_q_alpha + log_q_tau + log_q_nu + log_q_omega + log_q_sigma + log_q_a
  
  elbo
}

beta_update_ncr(X_matrix, y_vector, riesby$id, unique_ids, mu_nu_i_q, psi_ij_tau, psi_ij_omega, beta_prior)

nu_update_ncr(X_matrix, y_vector, unique_ids, id_indices, mu_beta_q, psi_ij_tau, psi_ij_omega, psi_i_alpha, mu_nu_i_q)

alpha_update(uis = u_matrix, psi_i_alpha, mu_nu_i_q, Sigma_nu_i_q, mu_alpha_q, alpha_prior)

tau_update_ncr(X_matrix, y_vector, w_matrix, riesby$id, unique_ids, mu_nu_i_q, Sigma_nu_i_q, 
           mu_beta_q, Sigma_beta_q, psi_ij_tau, psi_ij_omega, mu_tau_q, tau_prior)

omega_update_ncr(X_matrix, y_vector, riesby$id, unique_ids, mu_beta_q, Sigma_beta_q, mu_nu_i_q, Sigma_nu_i_q, 
...
elbo_cal_ncr(X_matrix, y_vector, u_matrix, w_matrix, mu_beta_q, Sigma_beta_q, mu_nu_i_q, Sigma_nu_i_q, 
